...
export default function OTP(){
    ...
    const handlesubmit = async () => {
      ...
      try {
        const res = await axios.post('https://ec2-43-204-100-120.ap-south-1.compute.amazonaws.com:4000/codenest/auth/v1/validateOtp', data);
        if (res.data) {
          setInvalid(false);
          setLoading(false);
          setValid(true);
    ...
    const handleResend = async () => {
      if(cnt==0){
        ...
        await axios.post('https://ec2-43-204-100-120.ap-south-1.compute.amazonaws.com:4000/codenest/auth/v1/register', {
          username: username,
          email:email,
          password:pswd
        }).then((res) => {
